...
namespace NightScreenViewerBackend
{
    ...
    {
        // 调整窗体不透明度
        public static async Task AdjustOpacityAsync(Form form, double targetOpacity, CancellationToken cancellationToken)
        {
            ...
            while (Math.Abs(form.Opacity - targetOpacity) > 0.03)
            {
                ...
                form.Invoke(new Action(() => 
                {
                    ...
                    NativeMethods.SetLayeredWindowAttributes(hWnd, 0, opacityByte, NativeMethods.LWA_ALPHA);
                }));
